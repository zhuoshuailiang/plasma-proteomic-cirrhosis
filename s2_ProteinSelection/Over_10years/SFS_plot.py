...
def get_nb_f(mydf):
    ...
    while((p_lst[i]<0.05)|(p_lst[i+1]<0.05)):
        i+=1
    return i
dpath = 'C:/Users/Administrator/Desktop/pro/shuai/Results/Over10Years/'
output_img_dir = dpath + 'AccAUC_TotalGain.png'
pro_imp_df = pd.read_csv(dpath + 'ProImportance.csv')
pro_imp_df.rename(columns = {'TotalGain_cv': 'Pro_imp'}, inplace = True)
pro_auc_df = pd.read_csv(dpath + 'AccAUC_TotalGain.csv')
...
ax.set_xticklabels(mydf['Pro_code'], rotation=90, fontsize=10, horizontalalignment='center')
my_col = ['r']*nb_f + ['k']*(len(mydf)-nb_f)
for ticklabel, tickcolor in zip(plt.gca().get_xticklabels(), my_col):
    ticklabel.set_color(tickcolor)

ax.set_ylabel('Predictor Importance', fontsize=18,labelpad=35)
...
ax2.set_ylabel('Cumulative AUC', fontsize=18, rotation=270,labelpad=35)
